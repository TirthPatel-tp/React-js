...
const News =(props)=> {
 ...
 const updateNews =  async() => {
  ...
  useEffect(()=> {
  document.title = `${capitalizeFirstLetter(props.category)} - Bharat`
      updateNews();
  ...
  const fetchMoreData = async () => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=3a7c8e8863034df09f5949f3fb9ab91c&page=${page+1}&pageSize=${props.pageSize}`;
    setPage(page+1)
    let data = await fetch(url);
    let parsedData = await data.json()
    ...
    return (
<>
      <h1 className='text-center' style={{ margin: '35px 0px' , marginTop:"90px"}}>Bharat - Top {capitalizeFirstLetter(props.category)} Headlines</h1>
